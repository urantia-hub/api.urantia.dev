...
describe("GET /audio/:paragraphId", () => {
	it("returns 200 for valid globalId ref", async () => {
		const res = await get("/audio/1:2.0.1");
		expect(res.status).toBe(200);
	});

	it("response has { data: { paragraphId, audio } }", async () => {
		const res = await get("/audio/1:2.0.1");
		const { data } = await res.json();
		expect(Object.keys(data).sort()).toEqual(["audio", "paragraphId"].sort());
		expect(data.paragraphId).toBeString();
	});
